...
class ChangeSet(models.Model):
    ...
    def map_platform_to_vagrant_box(platform):
        ...
        if platform:
            return map[platform.name] if platform.name and platform.name in map else default
        else:
            return default
    ...
    def create_topology_graph(self):
        graph = graphviz.Graph(name="topology", comment="Autogenerated using Netbox")
        seen_devices = []
        for device in Device.objects.all():  # TODO apply filtering
            attributes = {
            ...
            for interface in device.interfaces.all():
                if interface.form_factor not in NONCONNECTABLE_IFACE_TYPES + AGGREGATABLE_IFACE_TYPES:
                    peer_interface = interface.trace()[0][2]
                    if peer_interface and peer_interface.device not in seen_devices:
                        graph.edge(device.name + ":" + interface.name, peer_interface.device.name + ":" + peer_interface.name)
        print(graph.source)
    ...
    def to_actions(self):
        ...
        actions.append({
            'file_path': 'topology.dot',
            'content': self.create_topology_graph()
        })
        self.revert()
        return actions
