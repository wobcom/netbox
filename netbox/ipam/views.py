...
class OverlayNetworkGroupListView(generic.ObjectListView):
...
class OverlayNetworkGroupCreateView(generic.ObjectEditView):
...
class OverlayNetworkGroupBulkImportView(generic.BulkImportView):
    permission_required = 'ipam.add_overlaynetworkgroup'
...
class OverlayNetworkGroupBulkDeleteView(generic.BulkDeleteView):
...
class OverlayNetworkGroupOverlayNetworksView(View):
    def get(self, request, pk):
        ...
        paginate = {
            'paginator_class': EnhancedPaginator,
            'per_page': request.GET.get('per_page', get_paginate_count(request))
        }
...
class OverlayNetworkListView(generic.ObjectListView):
...
class OverlayNetworkCreateView(generic.ObjectEditView):
...
class OverlayNetworkDeleteView(generic.ObjectDeleteView):
    model = OverlayNetwork
    default_return_url = 'ipam:overlay_network_list'


class OverlayNetworkBulkImportView(generic.BulkImportView):
    model_form = forms.OverlayNetworkCSVForm
    table = tables.OverlayNetworkTable
    default_return_url = 'ipam:overlay_network_list'


class OverlayNetworkBulkEditView(generic.BulkEditView):
    queryset = OverlayNetwork.objects.select_related('site', 'group', 'tenant', 'role')
...
class OverlayNetworkBulkDeleteView(generic.BulkDeleteView):
    queryset = OverlayNetwork.objects.select_related('site', 'group', 'tenant', 'role')
    filter = filters.OverlayNetworkFilterSet
    table = tables.OverlayNetworkTable
    default_return_url = 'ipam:overlay_network_list'
